# following the tutorial "https://www.learnpytorch.io/06_pytorch_transfer_learning/"
...
image_transform = transforms.Compose(
    [
        ...
        PretainedModelParamsPreProc(ResNet50_Weights.DEFAULT.transforms),
    ]
)
...
class Cifar100ClfPretained(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        _weights = torchvision.models.EfficientNet_B0_Weights.DEFAULT
        self._pretained_model = torchvision.models.efficientnet_b0(weights=_weights).to(
            device
        )

        clf_in_feats = list(self._pretained_model.children())[-1][1].in_features

        self._fc1 = nn.Sequential(
            nn.Dropout(p=0.5), nn.Linear(clf_in_feats, 512), nn.ReLU(),
        )
        self._fc2 = nn.Sequential(nn.Dropout(p=0.5), nn.Linear(512, 100), nn.ReLU())

        self._pretained_model.classifier = nn.Sequential(self._fc1, self._fc2)
...
def train_ep(model, train_ds_loader, loss_fn):
    model.train()
    ...
    for (x, y) in train_ds_loader:
        ...
        train_loss_sum += loss_i


        if y_pred.shape[1] == 1:
            acc_i = y_pred.round().eq(y_pred).sum().item() / len(y_pred)

        else:
            acc_i = torch.argmax(y_pred, dim=1).eq(y_pred).sum().item() / len(y_pred)

        train_acc_sum += acc_i

    return {
        "loss": train_loss_sum / len(train_ds_loader),
        "acc": train_acc_sum / len(train_ds_loader),
    }

def test_ep(model, test_ds_loader, loss_fn):
    model.eval()
    ...
    return {
        "loss": test_loss_sum / len(test_ds_loader),
        "acc": test_acc_sum / len(test_ds_loader),
    }